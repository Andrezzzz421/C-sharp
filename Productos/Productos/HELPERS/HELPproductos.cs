...
namespace Productos.HELPERS
{
    public class HELPproductos
    {
       ...
       public static List<ProductoDTO> OrdenarPorCompra(string orden, double precio = 0)
       {
            var lista = dao.GetDTO();
            switch((orden ?? "").Trim().ToLowerInvariant())
            {
                case "mayor":
                    return lista.OrderByDescending(p => p.PrecioCompra).ToList();

                case "menor":
                    return lista.OrderBy(p => p.PrecioCompra).ToList();

                case "igual":
                    return lista.Where(p => p.PrecioCompra == precio).ToList();

                default:
                    return lista;
            }
       }
        public static List<ProductoDTO> BuscarPorDisponibilidad(string estado)
        {
            var lista = dao.GetDTO();
            ...
            return lista.Where(p => p.Disponible == disponible).ToList();
        }
