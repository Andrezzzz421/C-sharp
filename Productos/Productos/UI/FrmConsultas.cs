...
namespace Productos.UI
{
    public partial class FrmConsultas : Form
    {
        ...
        private void btnPrecioCompra_Click(object sender, EventArgs e)
        {
            string orden = cmbFiltroCompra.SelectedItem?.ToString() ?? "";

            if (orden.ToLower() == "igual")
            {
                if (double.TryParse(txtValor.Text, out double precio))
                {
                    dgConsultas.DataSource = null;
                    dgConsultas.DataSource = HELPproductos.OrdenarPorCompra(orden, precio);
                }
                else
                {
                    MessageBox.Show("Por favor ingresa un número válido.");
            ...
            {
                dgConsultas.DataSource = null;
                dgConsultas.DataSource = HELPproductos.OrdenarPorCompra(orden);
            }
        }

        private void cmbFiltroCompra_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbFiltroCompra.SelectedItem != null && cmbFiltroCompra.SelectedItem.ToString() == "Igual")
            {
                label4.Visible = true;
        ...
        {
            string estado = cmbDisponible.Text;

            var resultado = HELPproductos.BuscarPorDisponibilidad(estado);
